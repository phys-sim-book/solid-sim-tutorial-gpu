...
struct InvFreeSimulator<T, dim>::Impl
{
    int n_seg;
...
template <typename T, int dim>
InvFreeSimulator<T, dim>::InvFreeSimulator() = default;

template <typename T, int dim>
InvFreeSimulator<T, dim>::~InvFreeSimulator() = default;

template <typename T, int dim>
InvFreeSimulator<T, dim>::InvFreeSimulator(InvFreeSimulator<T, dim> &&rhs) = default;

template <typename T, int dim>
InvFreeSimulator<T, dim> &InvFreeSimulator<T, dim>::operator=(InvFreeSimulator<T, dim> &&rhs) = default;

template <typename T, int dim>
InvFreeSimulator<T, dim>::InvFreeSimulator(T rho, T side_len, T initial_stretch, T K, T h_, T tol_, T mu_, T Mu_, T Lam_, int n_seg) : pimpl_{std::make_unique<Impl>(rho, side_len, initial_stretch, K, h_, tol_, mu_, Mu_, Lam_, n_seg)}
{
}
template <typename T, int dim>
InvFreeSimulator<T, dim>::Impl::Impl(T rho, T side_len, T initial_stretch, T K, T h_, T tol_, T mu_, T Mu_, T Lam_, int n_seg) : tol(tol_), h(h_), mu(mu_), Mu(Mu_), Lambda(Lam_), window(sf::VideoMode(resolution, resolution), "InvFreeSimulator")
{
    ...
    host_DBC.push_back((n_seg + 1) * (n_seg + 1));
...
template <typename T, int dim>
void InvFreeSimulator<T, dim>::run()
{
    assert(dim == 2);
    bool running = true;
...
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::step_forward()
{
...
template <typename T, int dim>
T InvFreeSimulator<T, dim>::Impl::screen_projection_x(T point)
{
    return offset + scale * point;
}
template <typename T, int dim>
T InvFreeSimulator<T, dim>::Impl::screen_projection_y(T point)
{
    return resolution - (offset + scale * point);
}
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::update_x(const DeviceBuffer<T> &new_x)
{
...
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::update_x_tilde(const DeviceBuffer<T> &new_x_tilde)
{
    inertialenergy.update_x_tilde(new_x_tilde);
}
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::update_v(const DeviceBuffer<T> &new_v)
{
    frictionenergy.update_v(new_v);
    device_v = new_v;
}
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::update_DBC_target()
{
...
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::update_DBC_stiff(T new_DBC_stiff)
{
    DBC_stiff = new_DBC_stiff;
    springenergy.update_k(new_DBC_stiff);
}
template <typename T, int dim>
void InvFreeSimulator<T, dim>::Impl::draw()
{
    device_x.copy_to(host_x);
...
template <typename T, int dim>
T InvFreeSimulator<T, dim>::Impl::IP_val()
{
...
template <typename T, int dim>
DeviceBuffer<T> InvFreeSimulator<T, dim>::Impl::IP_grad()
{
    return add_vector<T>(
...
template <typename T, int dim>
DeviceTripletMatrix<T, 1> InvFreeSimulator<T, dim>::Impl::IP_hess()
{
...
template <typename T, int dim>
DeviceBuffer<T> InvFreeSimulator<T, dim>::Impl::search_direction()
{
    DeviceBuffer<T> dir;
...
template class InvFreeSimulator<float, 2>;
template class InvFreeSimulator<double, 2>;
template class InvFreeSimulator<float, 3>;
template class InvFreeSimulator<double, 3>;