...
template <typename T>
__device__ __host__ void PointEdgeDistanceTangent(const Eigen::Vector<T, 2>& p, const Eigen::Vector<T, 2>& e0, const Eigen::Vector<T, 2>& e1, Eigen::Matrix<T, 2, 1>& n, T& r) {
    Eigen::Vector<T, 2> e = e1 - e0;
	r = e.dot(p - e0) / e.dot(e);
	if (r < 0) {
		n = p - e0;
	}
	else if (r > 1) {
...
template __device__ __host__ void PointEdgeDistanceTangent(const Eigen::Vector2f& p, const Eigen::Vector2f& e0, const Eigen::Vector2f& e1, Eigen::Vector2f& n, float& r);
template __device__ __host__ void PointEdgeDistanceTangent(const Eigen::Vector2d& p, const Eigen::Vector2d& e0, const Eigen::Vector2d& e1, Eigen::Vector2d& n, double& r);
