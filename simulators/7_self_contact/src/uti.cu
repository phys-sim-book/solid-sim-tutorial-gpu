...
{
    T vec_min = 0.0f;              // Result of the reduction
    T *d_out;                      // Device memory to store the result of the reduction
    cudaMalloc(&d_out, sizeof(T)); // Allocate memory for the result

    DeviceReduce().Min(a.data(), d_out, a.size());

    // Copy the result back to the host
    cudaMemcpy(&vec_min, d_out, sizeof(T), cudaMemcpyDeviceToHost);

    // Clean up
    cudaFree(d_out);
    return vec_min;
}
template float min_vector<float>(const DeviceBuffer<float> &a);
...
template <typename T>
__device__ T smallest_positive_real_root_quad(T a, T b, T c)
{
    T discriminant = b * b - 4 * a * c;
    if (discriminant < 0)
    {
        return std::numeric_limits<T>::max(); // No real roots
    }

    T sqrt_discriminant = sqrtf(discriminant);
    T root1 = (-b - sqrt_discriminant) / (2 * a);
    T root2 = (-b + sqrt_discriminant) / (2 * a);

    T smallest_positive_root = std::numeric_limits<T>::max();
    if (root1 > 0)
    {
        smallest_positive_root = root1;
    }
    if (root2 > 0 && root2 < smallest_positive_root)
    {
        smallest_positive_root = root2;
    }
...
template __device__ float smallest_positive_real_root_quad<float>(float a, float b, float c);
template __device__ double smallest_positive_real_root_quad<double>(double a, double b, double c);
template __device__ long double smallest_positive_real_root_quad<long double>(long double a, long double b, long double c);